...
import { buildAuthorityPrompt, getAuthoritySystemInstruction } from './prompt-builder.js';
...
import { AUTHORITY_THEME } from '../../shared/themes/authority.theme.js';
...
export async function forgeAuthorityCarousel(
  sourceId?: string,
  manualTheme?: string,
  ...
  if (sourceId) {
    ...
    source = {
      ...
      category: manualSource?.category || 'industry_news',
    ...
    // Pick top unused source (any type, priority: relevance_score)
    const { data } = await supabase
      .from('sf_intel_sources')
      .select('*')
      .eq('used', false)
  ...
  if (!source) throw new Error('[FORGE-AUTHORITY] No intel source available');

  // 2. Build prompt
  const userPrompt = buildAuthorityPrompt({ source });
  const systemInstruction = getAuthoritySystemInstruction();
  ...
  const result = await retry(
    async () => {
      const response = await openai.chat.completions.create({
        model: OPENAI_MODEL,
        max_completion_tokens: 6144, // Reduced from 12288 (content-only output)
  ...
  const { data: post, error } = await supabase
    ...
    .insert({
      ...
      prompt_version: 'authority-v2', // New content-only version
      status: 'draft',
  ...
  if (error) throw new Error(`[FORGE-AUTHORITY] DB insert failed: ${error.message}`);
  ...
  logger.info(
    {
      ...
      promptVersion: 'authority-v2',
    },
    '[FORGE-AUTHORITY] Carousel forged successfully (content-only v2)'
  );
